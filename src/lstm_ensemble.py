...
from models import TextDataset, LSTMClassifier, train_model, custom_collate, \
    one_hot_encode, multiple_collate, TextSequenceModel, MultipleTextDataset
...
lemma_seq = ds['lemma'].tolist()
pos_seq_test = ds_test[POS].tolist()
lemma_seq_test = ds_test['lemma'].tolist()

print('before onehot')
pos_data, pos_test_data = one_hot_encode(pos_seq, pos_seq_test)
lemma_data, lemma_test_data = one_hot_encode(lemma_seq, lemma_seq_test)
...
dataset = MultipleTextDataset((pos_data, lemma_data), labels)
dataset_test = MultipleTextDataset((pos_test_data, lemma_test_data),
                                   labels_test)
...
train_loader = DataLoader(dataset, batch_size=BATCH_SIZE,
                          collate_fn=multiple_collate, sampler=train_sampler)
...
input_sizes = [pos_data[0].shape[1], lemma_data[0].shape[1]]
input_test_sizes = [pos_test_data[0].shape[1], lemma_test_data[0].shape[1]]
hidden_sizes = [32, 32]
num_layers = [1, 1]
mlp_hidden_size = 32
num_classes = 2
...
model = TextSequenceModel(input_sizes, hidden_sizes, num_layers,
                          mlp_hidden_size, num_classes).to(device=DEVICE)
